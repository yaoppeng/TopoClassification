...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        # print(self)
        self.load_state_dict(torch.load("/afs/crc.nd.edu/user/y/ypeng4/.cache/torch/hub/checkpoints/"
...
class TopoSwinTransformer(SwinTransformer):
    def __init__(self,
        ...
        self.topo_settting = topo_setting

    def update(self):
        self.pointnet_encoder = PointNetEncoder(global_feat=True, channel=4, feature_transform=False)

        self.linear_1 = nn.Sequential(nn.Linear(2048, 128), nn.ReLU())
        self.linear_2 = nn.Sequential(nn.Linear(2048, 256), nn.ReLU())
        self.linear_3 = nn.Sequential(nn.Linear(2048, 512), nn.ReLU())
        self.linear_4 = nn.Sequential(nn.Linear(2048, 1024), nn.ReLU())

        if self.topo_settting == 0:
            self.pointnet = PointNetEncoder(global_feat=True, channel=4)
        elif self.topo_settting == 1:
            self.pers_lay = PersLay()
        elif self.topo_settting == 2:
            self.pllay = PLLay()
        else:
            raise ValueError(f"invalid topo setting: {self.topo_settting}")
